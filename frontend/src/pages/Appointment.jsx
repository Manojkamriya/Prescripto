...
<div className='flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
  ...
  { // if their is docSlots length exist then only we show
    docSlots.length>0 && docSlots.map((item,index) =>(
      <div onClick={()=> setslotIndex(index)} className={`text-center py-6 min-w-16 rounded-full cursor-pointer 
        ...
        <p> {item[0] && daysOfWeek[item[0].datetime.getDay()] }</p>  {/* if 0th item available then we display daysOfWeek */} 
  ...
  <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4'>
    ...
    <p onClick={()=> setslotTime(item.time)} className={` text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer
      ${item.time === slotTime ? 'bg-primary text-white' : 'text-gray-400 border border-gray-300'} `} key={index}> {item.time.toLowerCase()} </p>
